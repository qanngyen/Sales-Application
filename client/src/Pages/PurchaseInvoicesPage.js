...
class PurchaseInvoices extends React.Component {
    ...
    async componentDidMount() {
        try {
            ...
            const response = await axios.get("/purchaseInvoices", {
    ...
    handleDeleteSalesInvoice = async (e, id) => {
        ...
        try {
            const result = await axios.delete(`/purchaseInvoices/${id}`, 
                {headers: { Authorization: `Bearer ${token}` }}
            );
            console.log(result);
            this.setState((prevState) => ({
                salesInvoices: prevState.salesInvoices.filter((invoice) => invoice.HDN_ID !== id),
            }));
        } catch (error) {
            console.log(error);
        }
